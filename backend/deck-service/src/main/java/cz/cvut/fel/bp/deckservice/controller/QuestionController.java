...
public class QuestionController {
    ...
    /**
     * Bulk update, create, and delete questions for a deck.
     */
    @PreAuthorize("hasRole('USER')")
    @PutMapping("/bulk")
    public ResponseEntity<List<QuestionResponseDTO>> bulkUpdateQuestions(
            @AuthenticationPrincipal UserPrincipal userPrincipal,
            @PathVariable Long deckId,
            @Valid @RequestBody BulkQuestionRequestDTO bulkRequest) {

        log.info("Bulk update questions request userId={}, deckId={}, updates={}, creates={}",
                userPrincipal.id(), deckId, bulkRequest.updates().size(), bulkRequest.creates().size());

        List<QuestionResponseDTO> responses = questionFacade.bulkUpdateQuestions(deckId, bulkRequest, userPrincipal);
