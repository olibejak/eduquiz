...
@CircuitBreaker(name = "flashcardsProgressApi")
@Slf4j
public class FlashcardProgressController {
    ...
    public ResponseEntity<Void> deleteDeck(@PathVariable Long deckId,
        ...
        log.debug("Delete deck progress request userId={}, deckId={}", userPrincipal.id(), deckId);
        flashcardProgressFacade.deleteDeckProgress(userPrincipal, deckId);
        log.debug("Delete deck progress retrieved userId={}", userPrincipal.id());
