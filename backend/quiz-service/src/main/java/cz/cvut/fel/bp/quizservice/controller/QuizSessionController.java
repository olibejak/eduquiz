...
@CircuitBreaker(name = "quizSessionApi")
@Slf4j
public class QuizSessionController {
    ...
    public ResponseEntity<Void> startSession(
            @PathVariable String pin,
            @RequestParam(defaultValue = "true") boolean hostPlaying,
            @RequestHeader("Quiz-Token") String token) {

        log.debug("action=startSessionRequested lobbyPin={} hostPlaying={}", pin, hostPlaying);
        lobbyFacade.validateHostAccess(pin, token);

        lobbyFacade.startSession(pin, hostPlaying);
