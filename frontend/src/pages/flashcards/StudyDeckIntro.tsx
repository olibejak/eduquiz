...
export default function StudyDeckIntro() {
    ...
    const fetchStatus = useCallback(() => {
        if (!user) {
            api.get(`/decks/${id}`)
                .then(res => {
                    const count = res.data.questions?.length || res.data.numberOfQuestions || 0;
                    setStatus({
                        title: "",
                        dueCount: count,
                        newCount: 0,
                ...
                .catch(err => console.error("Chyba při načítání sady pro hosta", err))
    ...
    return (
        <div className="max-w-2xl mx-auto p-6 mt-10 border rounded-2xl text-center shadow-sm"
            ...
            <p className="mb-8" style={{ color: 'var(--text)' }}>{user ? `Postup uživatele ${user.username}` :
                'K ukládání postupu se předem přihlašte'}</p>
