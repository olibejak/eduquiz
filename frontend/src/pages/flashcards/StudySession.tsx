...
export default function StudySession() {
    ...
    const submitBatchAndExit = async () => {
        ...
        navigate(user ? `/flashcards/${id}` : '/dashboard');
    };

    const handleRating = (rating: FlashcardRating) => {
        const currentQ = questions[currentIndex];
        const newReviews = [...reviews, { questionId: currentQ.id, rating }];
        ...
        if (currentIndex < questions.length - 1) {
            ...
            const hasToken = document.cookie.includes('jwt_token') || localStorage.getItem('jwt_token');
            if (hasToken) {
    ...
    return (
        <div className="flex flex-col md:flex-row overflow-hidden" style={{ backgroundColor: 'var(--bg)' }}>
            ...
            <div className="flex flex-row md:flex-col items-center md:items-stretch justify-between p-4 md:p-6 border-b md:border-b-0 md:border-r w-full md:w-64 flex-shrink-0" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--bg)' }}>
                ...
                >
                    Uložit <span className="hidden md:inline">a odejít</span>
                </button>
            </div>
                ...
                <div className="mt-6 mb-4 md:mt-10 md:mb-0 w-full flex justify-center">
                    {!showAnswer ? (
                        ...
                        <div className="flex flex-wrap justify-center gap-2 md:gap-4 w-full">
                            <button onClick={() => handleRating('AGAIN')} className="flex-1 md:flex-none px-4 md:px-6 py-3 md:py-2 rounded-lg font-bold bg-red-100 text-red-700 hover:bg-red-200 transition-colors text-sm md:text-base">
                                Znovu
                            </button>
                            <button onClick={() => handleRating('HARD')} className="flex-1 md:flex-none px-4 md:px-6 py-3 md:py-2 rounded-lg font-bold bg-orange-100 text-orange-700 hover:bg-orange-200 transition-colors text-sm md:text-base">
                                Těžké
                            </button>
                            <button onClick={() => handleRating('GOOD')} className="flex-1 md:flex-none px-4 md:px-6 py-3 md:py-2 rounded-lg font-bold bg-green-100 text-green-700 hover:bg-green-200 transition-colors text-sm md:text-base">
                                Dobré
                            </button>
                            <button onClick={() => handleRating('EXCELLENT')} className="flex-1 md:flex-none px-4 md:px-6 py-3 md:py-2 rounded-lg font-bold bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors text-sm md:text-base">
                                Výborné
                            </button>
                        </div>
                    )}
