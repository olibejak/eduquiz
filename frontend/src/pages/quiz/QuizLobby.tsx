...
export default function QuizLobby() {
    ...
    // derive whether the current client is host from router flag or from fetched participants
    const isCurrentHost = Boolean(
        isHost ||
        lobbyData?.participants?.some(p => String(p.id) === String(participantId) && (p.role === 'HOST' || p.role === 'ROLE_HOST'))
    );
    ...
    useEffect(() => {
        if (!events.hostEvent) return;

        const ev = events.hostEvent as { newHostId?: number; hostId?: number; participantId?: number; id?: number };
        const newHostId = Number(ev.newHostId ?? ev.hostId ?? ev.participantId ?? ev.id);
            ...
            const t = window.setTimeout(() => {
                setLobbyData(prev => {
                    ...
                    const updatedParticipants = (prev.participants || []).map(p => {
                        if (p.id === newHostId) return { ...p, role: 'HOST' };
                        if (p.role === 'HOST' || p.role === 'ROLE_HOST') return { ...p, role: 'USER' };
                        return p;
                    });
                    return { ...prev, participants: updatedParticipants };
                });
    ...
    }, [events.hostEvent, participantId]);
    ...
    useEffect(() => {
        if (
            ...
            navigate(`/quiz/${pin}/play`, {
                state: { isHost: isCurrentHost, participantToken, participantId }
            });
        }
    }, [events.gameStarted, events.currentQuestion, lobbyData?.currentState, navigate, pin, isCurrentHost, participantToken, participantId]);
    useEffect(() => {
    ...
    const handleStartQuiz = async () => {
        try {
            const quizApiLocal = createQuizApiLocal(participantToken);
            await quizApiLocal.post(`/quiz/${pin}/start?hostPlaying=${isHostPlaying}`);
        } catch (err) {
