...
export default function QuizPlay() {
    ...
    const correctAnswerIds = fullAnswers
        .filter((ans: any) => ans.payload?.isCorrect === true || ans.isCorrect === true)
        .map((ans: any) => ans.id);
    ...
    if (question && question.id !== prevQuestionId) {
        ...
        if (question.answers) {
            ...
            const terms = question.answers.filter((a: any) => a.associate === true);
            const defs = question.answers.filter((a: any) => a.associate === false || a.associate == null);
    ...
    const handleTermClick = (id: number) => {
        ...
        if (matchingPairs[id]) {
            ...
            // Výběr / Zrušení výběru levého pojmu
            setSelectedTermId(selectedTermId === id ? null : id);
        }
    };

    const handleDefClick = (id: number) => {
        if (isLocked) return;
        const existingTermId = Object.keys(matchingPairs).find(k => matchingPairs[Number(k)] === id);
        if (existingTermId) {
            const newPairs = { ...matchingPairs };
            delete newPairs[Number(existingTermId)];
            setMatchingPairs(newPairs);
        }

        if (selectedTermId !== null) {
            setMatchingPairs(prev => ({ ...prev, [selectedTermId]: id }));
            setSelectedTermId(null);
        }
    };
                        ...
                        <div className="w-full">
                            ...
                            {isMatching && (
                                <div className="flex flex-col gap-4 md:gap-6 w-full">
                                    <div className="text-center font-bold mb-2 text-[var(--text)] bg-[var(--accent-bg)] py-2 rounded-lg text-sm md:text-base shadow-sm">
                                        Nejprve vyberte pojem vlevo a následně k němu přiřaďte definici vpravo.
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 md:gap-8">
                                        <div className="flex flex-col gap-3">
                                            {shuffledTerms.map((term) => {
                                                ...
                                                const isPaired = matchingPairs[term.id] !== undefined;
                                                const pairColorIndex = Object.keys(matchingPairs).indexOf(String(term.id));
                                                const borderColors = ['#3b82f6', '#ef4444', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899'];
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        disabled={isLocked}
                                                        style={{
                                                            borderColor: isPaired ? borderColors[pairColorIndex % borderColors.length] : undefined,
                                                            borderWidth: isPaired || isSelected ? '4px' : '2px'
                                                        }}
                                                        className={`p-3 md:p-4 rounded-xl font-bold text-sm md:text-base text-left flex items-center min-h-[80px] transition-all bg-[var(--bg)] text-[var(--text-h)]
                                                            ${isSelected ? 'border-[var(--accent)] scale-[1.02] shadow-md bg-[var(--accent-bg)]' : ''}
                                                            ${!isLocked ? 'cursor-pointer hover:bg-[var(--accent-bg)] active:scale-95' : 'cursor-not-allowed opacity-90'}
                                                            ${!isSelected && !isPaired ? 'border-[var(--border)]' : ''}
                                                        `}
                                                    >
                                                        {term.text}
                                                    </button>
                                        ...
                                        <div className="flex flex-col gap-3">
                                            {shuffledDefs.map((def) => {
                                                const connectedTermId = Object.keys(matchingPairs).find(k => matchingPairs[Number(k)] === def.id);
                                                const isPaired = connectedTermId !== undefined;
                                                const pairColorIndex = isPaired ? Object.keys(matchingPairs).indexOf(connectedTermId!) : -1;
                                                const borderColors = ['#3b82f6', '#ef4444', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899'];
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        style={{
                                                            borderColor: isPaired ? borderColors[pairColorIndex % borderColors.length] : undefined,
                                                            borderWidth: isPaired ? '4px' : '2px'
                                                        }}
                                                        className={`p-3 md:p-4 rounded-xl font-bold text-sm md:text-base text-left flex items-center min-h-[80px] transition-all bg-[var(--bg)] text-[var(--text-h)]
                                                            ${isPaired ? 'opacity-90' : 'border-[var(--border)]'}
                                                            ${!isLocked && selectedTermId !== null && !isPaired ? 'cursor-pointer hover:border-[var(--accent)] hover:shadow-md animate-pulse' : ''}
                                    ...
                                    <button
                                        ...
                                        className={`w-full max-w-md mx-auto py-4 rounded-xl font-black text-lg md:text-xl transition-transform mt-6 bg-[var(--accent)] text-[var(--bg)] ${!isLocked && Object.keys(matchingPairs).length > 0 ? 'active:scale-95 cursor-pointer hover:opacity-90 shadow-lg' : 'opacity-50 cursor-not-allowed'}`}
                                    >
                                        Odeslat spojení ({Object.keys(matchingPairs).length} / {shuffledTerms.length})
                                    </button>
                                </div>
                            )}
                        </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 w-full max-w-4xl">
                                {Object.values(
                                    fullAnswers.reduce((acc: any, ans: any) => {
                                        const mId = ans.matchId ?? ans.payload?.matchId;
                                        if (mId != null) {
                                            if (!acc[mId]) acc[mId] = [];
